...
export function generateStaticParams(): { slug: string }[] {
  return getProgrammaticSlugs('erp').map(slug => ({ slug }))
}
...
export async function generateMetadata({
  ...
  const doc = getProgrammaticDoc('erp', slug)
  if (!doc) return {}

  return {
    title: doc.title,
    description: doc.description,
    alternates: { canonical: `/erp/${slug}` },
  }
}
...
export default async function ErpProgrammaticPage({
  params,
}: {
  params: Promise<{ slug: string }>
}) {
